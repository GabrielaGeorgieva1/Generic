...
public class DataTypesAndMethodsDemo {
    ...
    public static void main(String[] args) {
        ...
        increment(z);
        System.out.println("From main, after increment: " + z);

        int[] arr = {1, 2, 3, 4, 5};
        // when we pass `arr` as argument a copy of the reference is created
        // the copy of the reference points to the same array as `arr`
        int[] updatedArray = update(arr);
        System.out.println("arr from main, after update: " + Arrays.toString(arr));
        System.out.println("updatedArray from main, after update: " + Arrays.toString(updatedArray));
    }

    static int[] update(int[] array) {
        for (int i = 0; i < array.length; i++) {
            // here me modify the value of the same array that is pointed by `arr`
            array[i] *= 10;
        }
        System.out.println("From update, after * 10: " + Arrays.toString(array));
        ...
        System.out.println("From update, after new array and array[0] * 10: " + Arrays.toString(array));
    ...
    static void increment(int x) {
        ...
        x += 1;
        System.out.println("In increment, after +1: " + x);
    }
